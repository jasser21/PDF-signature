...
const props: { pdfUrl?: string } = defineProps({
  pdfUrl: String, // URL of the PDF file hosted from the API
});
...
async function fetchPdffromapi() {
  try {
    const response = await fetch(props.pdfUrl as string);
    if (!response.ok) {
      throw new Error(`Failed to fetch PDF: ${response.status}`);
    }

    const blob = await response.blob();
    const filename = response?.headers?.get('Content-Disposition')?.split('filename=')[1] || 'defaultName';
    const file = new File([blob], filename, { type: 'application/pdf' });

    await readerFile(file); // Call readerFile with the obtained File object
  } catch (error) {
...
async function readerFile(files?: FileList | File | null) {
  try {
    const regexp = /.pdf|.png|.jpg|.jpeg/;
    console.log(files);
    let file;
    if (files instanceof FileList) {
      file = checkFile(files, regexp);
    }
...
<template>
  <div class="upload_content content">
    ...
    <div v-show="fileName" class="upload_content_box pt-3 md:pt-12">
      ...
      <div class="w-full flex flex-col gap-4 items-center">
        ...
        <label class="w-[90%] relative max-w-[400px]">
          <input v-model.trim="projectName" type="text" class="input" @focus="focus" @blur="blur" />
          <img src="@/assets/icon/ic_edit.svg" alt="edit icon" :class="[
    ...
    <div v-if="!fileName" class="upload_content_box border-dashed border-secondary border-[1px] justify-center"
      @dragover.stop.prevent @dragenter.stop.prevent @drop.stop.prevent="dropFile">
      <img src="@/assets/img/img_photo.svg" alt="photo icon" />
      <button class="btn btn_primary">
        <input type="file" accept="application/pdf, .jpg, .png"
          class="opacity-0 absolute w-[131px] h-[41px] cursor-pointer" @change="uploadFile" />{{ $t('select_file') }}
      </button>
    ...
    <sign-step-btn :is-next-disabled="isNextDisabled" @next-step="goPage('signature')"
    ...
    <sign-popup v-if="isShowWarnPopup" :title="$t('warn')">
      ...
      <div class="flex justify-between md:justify-around">
        <button class="btn btn_base" @click="toggleWarnPopup(false)">
          {{ $t('not_yet') }}
        </button>
        <button class="btn btn_primary" @click="giveUpUpload">
          {{ $t('give_up') }}
        </button>
      </div>
    </sign-popup>
...
.upload_content {
  &_box {
    @apply my-5 px-[10px] rounded-[20px] flex flex-col items-center gap-7 overflow-y-auto w-full h-[calc(100%-128px)];
  }
}
</style>
